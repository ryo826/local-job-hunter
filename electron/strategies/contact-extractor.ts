...
export class ContactExtractor {
     ...
     * 企業HPから連絡先情報を抽出
     */
    async extract(page: Page, companyUrl: string, onLog?: (message: string) => void): Promise<ContactInfo> {
        const log = (msg: string) => {
            if (onLog) onLog(msg);
            else console.log(`[ContactExtractor] ${msg}`);
        };
     ...
     * メールアドレスを抽出
     */
    private async extractEmail(page: Page, log: (msg: string) => void): Promise<string | undefined> {
        try {
            // 1. mailto:リンクから抽出
            ...
            // スパム対策テキストの変換
            const converted = emails.map(email =>
                email
                    .replace(/\[at\]/g, '@')
                    .replace(/\[dot\]/g, '.')
                    .replace(/（at）/g, '@')
