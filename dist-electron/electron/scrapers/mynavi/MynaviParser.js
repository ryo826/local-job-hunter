...
exports.MynaviParser = void 0;
class MynaviParser {
    parseJsonLd(jobId, jsonLd, html) {
        return {
            // 統合ID
            id: `mynavi_${jobId}`,
            source: 'mynavi',
    ...
    summarizeLocations(jobLocation) {
        const locs = this.parseLocations(jobLocation);
        if (locs.length === 0)
            return '';
        return locs.map(loc => [loc.region, loc.locality].filter(Boolean).join(' ')).join(', ');
    }
    cleanHtml(html) {
        if (!html)
            return '';
...
exports.MynaviParser = MynaviParser;
