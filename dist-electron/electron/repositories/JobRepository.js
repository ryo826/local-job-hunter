...
class JobRepository {
     ...
     */
    getByUrl(url) {
        const row = this.db.prepare('SELECT * FROM jobs WHERE source_url = ?').get(url);
        return row ? this.deserialize(row) : null;
    }
    /**
     * 求人が存在するかチェック
    ...
    update(job) {
        const stmt = this.db.prepare(`
            ...
            WHERE id = ?
        `);
        stmt.run(job.companyName, job.companyUrl, job.companyLogo, job.title, job.employmentType, job.industry, job.description, job.requirements, job.benefits, job.workHours, job.salaryMin, job.salaryMax, job.salaryText, JSON.stringify(job.locations), job.locationSummary, JSON.stringify(job.labels), JSON.stringify(job.keywords), job.datePosted, job.dateExpires, job.dateUpdated, job.lastCheckedAt, job.isActive ? 1 : 0, JSON.stringify(job.ngKeywordMatches || []), job.id);
    }
    /**
     * last_checked_atのみ更新
     */
    updateLastChecked(id, timestamp) {
    ...
    getStats() {
        ...
        const active = this.db.prepare('SELECT COUNT(*) as count FROM jobs WHERE is_active = 1').get().count;
